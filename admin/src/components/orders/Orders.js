                            ...
                            {items.map((item, index) => (
                              <div key={index} className="order-item">
                                ...
                                <div className="item-details">
                                  ...
                                  <p>Price: {formatCurrency(isVendor ? (item.vendorDisplayUnitPrice ?? item.vendorUnitSpecialPrice ?? item.vendorUnitPrice ?? item.price) : item.price)}</p>
                                </div>
                                <div className="item-total">
                                  {formatCurrency(((isVendor ? (item.vendorLineTotal ?? ((item.vendorDisplayUnitPrice ?? item.vendorUnitSpecialPrice ?? item.vendorUnitPrice ?? item.price) * item.quantity)) : (item.price * item.quantity))))}
                                </div>
                              </div>
                            ))}
                          </div>
